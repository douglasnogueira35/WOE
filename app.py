...
if "data_ref" in df.columns:
    ...
    st.subheader("📅 Distribuição por Mês de Referência")
    tabela_mensal = df["data_ref"].dt.to_period("M").value_counts().sort_index()
    st.bar_chart(tabela_mensal)

    # Split temporal: treino até um mês, teste nos meses seguintes
    df = df.sort_values("data_ref")
    limite_treino = "2023-12-31"  # ajuste conforme sua base
    treino = df[df["data_ref"] <= limite_treino]
    teste = df[df["data_ref"] > limite_treino]

    X_train, y_train = treino.drop(columns=[alvo]), treino[alvo]
    X_test, y_test = teste.drop(columns=[alvo]), teste[alvo]
else:
    # fallback se não houver coluna de data
...
for nome, modelo in modelos.items():
    try:
        ...
        variaveis_importancia[nome] = df_imp
...
st.download_button("⬇️ Baixar CSV", csv, "relatorio_modelos.csv")
...
with pd.ExcelWriter(buffer, engine="xlsxwriter") as writer:
    ...
    for nome, df_imp in variaveis_importancia.items():
        df_imp.to_excel(writer, sheet_name=nome[:31])
...
for modelo, metricas in resultados.items():
    if isinstance(metricas, dict):
        pdf.cell(200, 10, txt=f"--- {modelo} ---", ln=True)
        for metrica, valor in metricas.items():
            pdf.cell(
                200, 10,
                txt=f"{metrica}: {valor:.4f}",
                ln=True
            )

# Corrigido: gerar PDF em memória
